...
output_task1 = output_directory + "task1_largest_discrepancy.csv"
output_task2 = output_directory + "task2_consistent_airlines.csv"
output_task3 = output_directory + "task3_canceled_routes.csv"
output_task4 = output_directory + "task4_carrier_performance_time_of_day.csv"
...
def find_largest_discrepancy(flights_data, carriers_data):
    ...
    merged_data = flights_data.join(
        carriers_data,
        flights_data["CarrierCode"] == carriers_data["CarrierCode"]
    ).select(
        "FlightNum", 
        carriers_data["CarrierName"],
        "Origin",
        "Destination",
        "scheduled_travel_duration",
        "actual_travel_duration",
        "time_discrepancy",
        flights_data["CarrierCode"]
    )
    ...
    ranked_data = merged_data.withColumn("rank", F.row_number().over(window_specification))

    # Filter to keep only the top-ranked discrepancies
    largest_discrepancy_data = ranked_data.filter(ranked_data["rank"] == 1).select(
        "FlightNum",
        "CarrierName",
        "Origin",
        "Destination",
...
def find_consistent_airlines(flights_data, carriers_data):
    ...
    ).filter("flight_count > 100")  # Only include carriers with more than 100 flights
    
    # Join with carriers to get carrier names
    delay_statistics = delay_statistics.join(
        carriers_data, delay_statistics["CarrierCode"] == carriers_data["CarrierCode"]
    ).select(
        carriers_data["CarrierName"],
        "flight_count",
...
# ------------------------
def find_canceled_routes(flights_data, airports_data):  # Corrected function name
...
def analyze_carrier_performance_time_of_day(flights_data, carriers_data):
    ...
    final_result = ranked_carriers.join(
        carriers_data, ranked_carriers.CarrierCode == carriers_data.CarrierCode
    ).select(
